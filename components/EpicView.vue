...
const stories = computed(() =>{
  return currentProject.value?.epics.find((epic) => epic.id === Number(epicId))?.stories
});
...
<template>
  <div class="w-full h-full max-h-full">
    <StackContainer direction="column" justify="stretch" class="w-full h-full max-h-full">
      <div class="my-4 w-full">
        <UAccordion v-if="currentProject?.epics[Number(epicId)]?.description" color="gray" variant="ghost" size="sm"
          :items="[
            {
              label: 'Description',
      ...
      <div v-if="stories && stories.length > 0" class="w-full h-full">
        <StackContainer direction="column" align-items="stretch" spacing="4">
          <StoryView v-for="story in stories" :key="story.id" :story="story" :project-id="Number(projectId)" />
        </StackContainer>
      </div>
