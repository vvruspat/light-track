...
export default defineEventHandler(
  async (event): Promise<EpicPutResponse> => {
    const body = await readValidatedBody<EpicPutRequest>(event, (b) => {
      const ajv = new Ajv();
      const schema = paths["/epics/{id}"]["put"]["requestBody"]["content"]["application/json"].schema;
    ...
    const { data: epic, error: selectEpicError } = await client.from("epics").select().eq("id", id).single();
