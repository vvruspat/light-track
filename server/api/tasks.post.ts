...
import { TaskPostRequest, TaskPostResponse } from "@/types/api";
import { paths } from "@/public/_openapi.json";
import Ajv from "ajv";

export default defineEventHandler(
  async (event): Promise<TaskPostResponse> => {
    const body = await readValidatedBody<TaskPostRequest>(event, (b) => {
      const ajv = new Ajv();
      const schema = paths["/tasks"]["post"]["requestBody"]["content"]["application/json"].schema;
